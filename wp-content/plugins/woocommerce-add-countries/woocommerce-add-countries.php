...
/*
Copyright 2014-2016  Daniele De Santis  (email : info@danieledesantis.net)
...
if(!class_exists('Wac'))
{
    class Wac
    {
		...
		public function woocommerce_assign_countries( $continents ) {
			$countries_list = get_option('wac_countries_list');
			if ($countries_list != '') {
				$countries_list = explode("\r\n", $countries_list);
				foreach($countries_list as $single_country) {
					$single_country = explode(",", $single_country);
					$iso_code = trim($single_country[0]);
					$continent_code = trim($single_country[2]);
					if ($continent_code != '') {
						$continents[$continent_code]['countries'][] = $iso_code;
					}					
				}
			}
			return $continents; 
		...
		public function apply_wac_filter() {
			...
			add_filter( 'woocommerce_continents', array($this, 'woocommerce_assign_countries'), 10, 1 );
		}
		...
		public function wac_settings_page() {
			...
			echo '<h2>' . __('Woocommerce Add Countries', 'woocommerce-add-countries') . '</h2>
					...
					. __('Enter the desired ISO code, country name and continent code separated by a comma.', 'woocommerce-add-countries') . '<br>' 
					. __('Enter each country on a new line.', 'woocommerce-add-countries') . '</p>
					<p>' . __('Example:','woocommerce-add-countries') . '</p>
					<p>' . __('UK, United Kingdom, EU','woocommerce-add-countries') . '<br>'
					. __('FR, France, EU','woocommerce-add-countries') . '<br>'
					. __('NAF, New African Country, AF','woocommerce-add-countries') . '<br>'
					. __('NAC, New African Country, AS','woocommerce-add-countries') . '</p>
					<p>' . __('Available continent codes:', 'woocommerce-add-countries') . '<br>'
					. __('AF (Africa), AN (Antarctica), AS (Asia), EU (Europe), NA (North America), OC (Oceania), SA (South America).', 'woocommerce-add-countries') . '</p>';
