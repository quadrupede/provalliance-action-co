        ...
        if ( !empty( $_POST['action'] ) && $_POST['action'] == 'save_to_address_book' ) {
            ...
            if ( count($errors) > 0 ) {
                if ( function_exists( 'wc_add_notice' ) ) {
                    ...
                    $woocommerce->add_error( __( 'Please enter the complete address', 'wc_shipping_multiple_address' ) );
                }
                ...
                foreach ($addresses as $addr) {
                    ...
                    if ($md5 == $addrMd5) {
                        ...
                        if ( function_exists( 'wc_add_notice' ) ) {
                            ...
                            $woocommerce->add_error( __( 'Address is already in your address book', 'wc_shipping_multiple_address' ) );
                        }
                        $next = add_query_arg( $address, $redirect_url );
                        wp_redirect( add_query_arg('address-form', 1, $next ) );
                        exit;
