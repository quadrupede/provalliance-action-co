...
class WC_MS_Notes {
    ...
    public function __construct( WC_Ship_Multiple $wcms ) {
        ...
        add_filter( 'woocommerce_checkout_fields', array( __CLASS__, 'register_delivery_notes_field' ) );
        ...
        add_filter( 'woocommerce_admin_shipping_fields', array(__CLASS__, 'render_delivery_notes_field') );
        add_filter( 'woocommerce_email_customer_details_fields', array(__CLASS__, 'email_delivery_notes_field'), 10, 3 );
        add_filter( 'woocommerce_order_details_after_customer_details', array(__CLASS__, 'order_delivery_notes_field'), 10 );
    }
    ...
    /**
     * Register the shipping_notes field for checkout forms that do not use multishipping
     *
     * @param array $fields
     * @return array
     */
    public static function register_delivery_notes_field( $fields ) {
        global $wcms;
        ...
        if ( $show_notes ) {
            ...
            $fields['shipping']['shipping_notes'] = array(
                'type'          => 'textarea',
                'input_class'   => array('ms_shipping_note'),
                'label'         => __( 'Delivery Notes', 'wc_shipping_multiple_address' ),
                'placeholder'   => __( 'Delivery Notes', 'wc_shipping_multiple_address' ),
            );

            if ( !empty( $limit ) ) {
                $fields['shipping']['shipping_notes']['description'] = sprintf( __('Character Limit: %d', 'wc_shipping_multiple_address'), $limit );
                $fields['shipping']['shipping_notes']['custom_attributes']['data-limit'] = $limit;
            }
    ...
    public static function render_delivery_notes_field( $fields ) {
        global $wcms;
    ...
    /**
     * Render the delivery notes field in order emails
     *
     * @param array     $fields
     ...
     * @return array
     */
    public static function email_delivery_notes_field( $fields, $sent_to_admin, $order ) {
        if ( isset( $order->shipping_notes ) ) {
            $fields['notes'] = array(
                'label' => __( 'Delivery Notes', 'wc_shipping_multiple_address' ),
                'value' => wptexturize( $order->shipping_notes )
            );
        }
    ...
    /**
     * Render the delivery notes field in order details views
     *
     * @param WC_Order  $order
     */
    public static function order_delivery_notes_field( $order ) {
        if ( isset( $order->shipping_notes ) ) {
            ?>
            <tr>
                <th><?php _e( 'Delivery Notes:', 'wc_shipping_multiple_address' ); ?></th>
                <td><?php echo esc_html( $order->shipping_notes ); ?></td>
            </tr>
            <?php
        }
    }
